...
- Walks you through four routes: **Fast-track** (BCS / highly-skilled / Foreign Interest company), **Standard**, **Marriage to a Cypriot citizen**, and an informational card for **M126** (registering a minor child as a Cypriot citizen).
...
- Tracks a full travel log (arrivals/departures) and automatically surfaces trips taken after the cumulative threshold is reached, with an option to add or edit trips manually.
...
- Generates a print-ready PDF summary (English, Greek, or both) that mirrors the official "Detailed Record of Arrivals and Departures" format, and lets you download it at any point in the flow — every field is optional.
- Ships in three interface languages: English, Russian (Русский), and Greek (Ελληνικά).

## Important disclaimer

This tool provides an **informal, unofficial estimate only** — it is not legal advice. It does not account for "visa gaps" (illegal periods, or periods without a valid visa, residence permit, or visa-free passport) and was not designed for such cases, even where an immigration officer indicated informally that a gap "wasn't a problem" (e.g., during the COVID-19 pandemic). If any of this applies to you, or if your case involves back-and-forth status changes, please consult a licensed Cyprus immigration lawyer instead of relying on this or any other online calculator.
